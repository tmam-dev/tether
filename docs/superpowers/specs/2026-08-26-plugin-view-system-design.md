...
- `sandbox="allow-scripts allow-same-origin"` — scripts run and the plugin can `fetch` same-origin (needed for §3.5's API), but no `allow-top-navigation`, no `allow-popups`, no `allow-forms`. This is defense in depth for a "you chose to install this" trust model, not a hostile-marketplace threat model — there's no user auth or cross-user data on a single local instance for a plugin to steal, but a plugin still shouldn't be able to navigate or pop the host window.
...
- **How a plugin is isolated from the host** — sandboxed same-origin iframe (§3.4), chosen over unsandboxed same-origin script (no isolation) and a Web Component contract (steeper authoring bar, ties plugin authors to a Tether-specific JS API instead of "just build a web page").
